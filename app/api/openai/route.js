...
export async function POST(req) {
  ...
  switch (action) {
    ...
    case "retrieve":
      ...
      const getRun = await openai.beta.threads[method].retrieve(threadId, runId);
      return NextResponse.json({ ...getRun });
    ...
    case "create":
      switch (method) {
        case "threads":
          ...
          return NextResponse.json({ ...getThread });
        case "messages":
          const getMessages = await openai.beta.threads[method].create(threadId, {
            role,
            content,
          });
